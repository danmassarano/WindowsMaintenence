...
This can be done by running ```SetupScheduler.ps1```. It's currently set to schedule a weekly run at 3am, so if you want to alter the times you can do so in ```config.json```

If you prefer to set it up manually, or make changes to the schedule, use the Task Scheduler (Win+R, and type ```taskschd.msc```).
...
In the triggers tab you can set a trigger for when this is executed - like on a certain time interval, or on an event like when you sign out or restert. I want this to just run on a weekly basis, and I know when I don't use this PC, so I set it to run on a Sunday night. 
